      ...
      <div class="p-8 flex flex-column gap-4 items-center ">
        <div class="flex flex-column gap-2 w-1/2">
          <el-form label-position="top">
            <el-form-item class="w-[100%]" v-for="field in data.form" :label="field.placeholder">
              <el-tooltip v-if="field.type == 'image'" content="Заргузите аватарку" placement="top">
                <el-upload
                    v-model="field.value"
                    :disabled="!field.canEdit"
                    :show-file-list="false"
                    :http-request="(image) => updateAvatar(image.file, route.params.id)"
            ...
            <el-form-item>
              <div class="flex flex-column w-full gap-2">
                <el-button v-if="!editMode && !!data.editMode" type="primary" size="large" class="w-full"
                           @click="editMode = true">Изменить
                </el-button>
                <el-button v-if="editMode" type="primary" size="large" class="w-full" @click="handleUpdateUser">
                  Сохранить
                </el-button>
                <el-button v-if="editMode" type="danger" size="large" class="w-full" @click="() => {
            formUser();
            editMode = false
          }">Отменить
                </el-button>
